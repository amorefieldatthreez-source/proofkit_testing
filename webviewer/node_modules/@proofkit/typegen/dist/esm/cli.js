...
async function runCodegen({ configLocation, resetOverrides = false }) {
  if (!fs.existsSync(configLocation)) {
  ...
  const { generateTypedClients } = await import("./typegen.js");
...
program.command("generate", { isDefault: true }).option("--config <filename>", "optional config file name").option("--env-path <path>", "optional path to your .env file").option(
  ...
  false
).option("--skip-env-check", "(deprecated) Ignore loading environment variables from a file.", false).action(async (options) => {
...
program.command("init").option("--config <filename>", "optional config file name").action((options) => {
  const configLocation = path.toNamespacedPath(path.resolve(options.config ?? defaultConfigPaths[0] ?? ""));
  console.log(configLocation);
  init({ configLocation });
});
program.command("ui").description("Launch the configuration UI").option("--port <number>", "Port for the UI server").option("--config <filename>", "optional config file name").option("--no-open", "Don't automatically open the browser").option("--env-path <path>", "optional path to your .env file").action(async (options) => {
  const configPath = getConfigPath(options.config);
