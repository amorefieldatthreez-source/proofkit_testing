...
const envNames = envNamesBase.transform((val) => {
  if (!val) {
    return void 0;
  }
  const transformed = {
    server: val.server === "" ? void 0 : val.server,
    db: val.db === "" ? void 0 : val.db,
    auth: val.auth ? {
      apiKey: val.auth.apiKey === "" ? void 0 : val.auth.apiKey,
      username: val.auth.username === "" ? void 0 : val.auth.username,
      password: val.auth.password === "" ? void 0 : val.auth.password,
      clarisIdUsername: val.auth.clarisIdUsername === "" ? void 0 : val.auth.clarisIdUsername,
      clarisIdPassword: val.auth.clarisIdPassword === "" ? void 0 : val.auth.clarisIdPassword
    } : void 0,
    fmMcp: val.fmMcp ? {
      baseUrl: val.fmMcp.baseUrl === "" ? void 0 : val.fmMcp.baseUrl,
      connectedFileName: val.fmMcp.connectedFileName === "" ? void 0 : val.fmMcp.connectedFileName
    } : void 0
  };
  if (transformed.auth && Object.values(transformed.auth).every((v) => v === void 0)) {
    transformed.auth = void 0;
  }
  if (transformed.fmMcp && Object.values(transformed.fmMcp).every((v) => v === void 0)) {
    transformed.fmMcp = void 0;
  }
  if (Object.values(transformed).every((v) => v === void 0)) {
    return void 0;
  }
  return transformed;
}).meta({
...
const path = z.string().default("schema").optional().meta({ description: "The folder path to output the generated files" });
const fieldOverride = z.object({
  ...
  typeOverride: z.enum([
    ...
    // numberField()
    "boolean",
    // numberField().outputValidator(z.coerce.boolean())
    "date",
    // dateField()
    "timestamp",
    // timestampField()
...
const validatorField = z.union([z.enum(["zod", "zod/v4", "zod/v3"]), z.literal(false)]).default("zod/v4").optional().meta({
...
const fmMcpFieldObject = z.object({
  ...
  connectedFileName: z.string().optional().meta({
    description: "Name of the connected FileMaker file. If not provided, it will be auto-discovered from the FM MCP server's /connectedFiles endpoint and written back to your config. Can also be set via FM_CONNECTED_FILE_NAME env var."
  })
});
const fmMcpField = z.preprocess((val) => {
  if (val === true) {
    return { enabled: true };
  }
  return val;
}, fmMcpFieldObject).optional().meta({
...
const createFmdapiConfig = (envNamesSchema) => z.object({
...
const createFmodataConfig = (envNamesSchema) => z.object({
...
const typegenConfigSingleForValidation = z.discriminatedUnion("type", [
  createFmdapiConfig(envNamesBase),
  createFmodataConfig(envNamesBase)
]);
const typegenConfigSingle = z.preprocess((data) => {
  if (data && typeof data === "object" && !("type" in data)) {
    return { ...data, type: "fmdapi" };
  }
  return data;
}, typegenConfigSingleBase);
const typegenConfig = z.object({
...
const typegenConfigForValidation = z.object({
  postGenerateCommand: z.string().optional().meta({
    description: "Optional CLI command to run after files are generated. Commonly used for formatting. Example: 'pnpm biome format --write .' or 'npx prettier --write src/'"
  }),
